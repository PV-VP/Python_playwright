...
def test_create_task(page: Page, open_odoo):
    ...
    page.get_by_text("Казначейство", exact=True).click()
    page.get_by_text("Пошук...").blur()                                         #убираєм фокус з пошуку
    page.locator("button.creation_form_tabs_item", has_text="Загальні").click()
    page.get_by_placeholder("Вкажіть назву задачі").fill(generate_task_name())
    page.locator("span.vsd_placeholder", has_text="Виберіть значення").click()
    page.locator(".selection_item", has_text="Низький").click()
